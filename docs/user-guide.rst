...
The ``acumos_model_runner`` package installs a command line tool ``acumos-model-runner`` for running models created by the `Acumos Python client library <https://pypi.org/project/acumos/>`__.
...
.. code:: bash
    ...
    $ acumos-model-runner --help
    usage: acumos-model-runner [-h] [--host HOST] [--port PORT]
