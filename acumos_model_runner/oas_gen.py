...
def create_oas(metadata, protobuf):
    ...
    methods = [dict(name=name, **method) for name, method in metadata['methods'].items()]
...
def _create_definitions(top_level):
    ...
    definitions = {name: definition for item in top_level for name, definition in _find_definitions(item, refs)}
    return definitions
...
def _resolve_type(type_name, refs, prefix):
    ...
    if type_name in _PROTO_OAS_MAP:
        ...
        ref_str = "#/definitions/{}".format(".".join(ref_tuple))
        return {'$ref': ref_str}
