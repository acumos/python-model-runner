...
def model():
    ...
    model = Model(add=add, count=count, empty=empty, rotate_image=rotate_image)
    return model
...
def test_runner(model):
    ...
    with _run_model(model) as runner:
        ...
        resp_raw = api._post_octet_stream('rotate_image', data=raw_input)
        assert resp_raw == raw_output
