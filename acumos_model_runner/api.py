...
def methods(method_name):
    ...
    if content_type == _PROTO:
        try:
            wrapped_resp = method.from_pb_bytes(data)
        except DecodeError:
            raise abort(Response('Could not decode input protobuf message. Verify that the correct protobuf message type is being used', 400))
    else:
        wrapped_resp = method.from_json(data)
